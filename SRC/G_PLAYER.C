...
Player_t
g_create_player (short start_x, short start_y, u_char *controller)
{
  Player_t p;
...
void
g_handle_player_input (Player_t *p)
{
  int padd;
  ...
  if (padd & PADRdown)
    p->position.vy -= JUMP_VELOCITY * s_delta_time () / 4096;
}

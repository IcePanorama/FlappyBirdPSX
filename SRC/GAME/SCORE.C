...
static PlayerEntity_t *player_ptr;
...
s_init_scoring (PlayerEntity_t *p)
{
  ...
  i16_green_area_start =
    -((player_ptr->u8_width + (PIE_HALF_PIPE_WIDTH)) << (FB_WORLD_TO_CAMERA_SPACE_NUM_SHIFTS));
...
s_process_scoring (uint8_t u8_eid)
{
  ...
  if (u8_eid == 0) return;      // Ignore player_ptr entity
